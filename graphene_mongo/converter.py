    ...
    if isinstance(base_type, graphene.Field):
        if isinstance(field.field, mongoengine.GenericReferenceField):
            ...
            async def get_reference_objects_async(*args, **kwargs):
                ...
                document_field = mongoengine.ReferenceField(document)
                document_field = convert_mongoengine_field(document_field, registry, executor=ExecutorEnum.ASYNC)
                document_field_type = document_field.get_type().type
                queried_fields = list()
                filter_args = list()
                if document_field_type._meta.filter_fields:
            ...
            async def reference_resolver_async(root, *args, **kwargs):
                ...
                if to_resolve:
                    ...
                    for model, object_id_list in choice_to_resolve.items():
                        if model in to_resolve_models:
                            task = loop.create_task(get_reference_objects_async(model, object_id_list, registry, args))
                        else:
            ...
            return graphene.List(
                ...
                required=field.required,
                resolver=reference_resolver if executor == ExecutorEnum.SYNC else reference_resolver_async
            )
        return graphene.List(
            base_type._type,
            description=get_field_description(field, registry),
...
def convert_field_to_union(field, registry=None, executor: ExecutorEnum = ExecutorEnum.SYNC):
    ...
    if isinstance(field, mongoengine.GenericLazyReferenceField):
        ...
        return graphene.Field(_union, resolver=field_resolver if field_resolver else (
            lazy_reference_resolver if executor == ExecutorEnum.SYNC else lazy_reference_resolver_async),
...
def convert_field_to_dynamic(field, registry=None, executor: ExecutorEnum = ExecutorEnum.SYNC):
    ...
    def dynamic_type():
        ...
        if isinstance(field, mongoengine.ReferenceField):
            return graphene.Field(_type, resolver=field_resolver if field_resolver else (
                reference_resolver if executor == ExecutorEnum.SYNC else reference_resolver_async),
                                  description=get_field_description(field, registry), required=required)
        else:
            return graphene.Field(_type, resolver=field_resolver if field_resolver else (
                cached_reference_resolver if executor == ExecutorEnum.SYNC else cached_reference_resolver_async),
                                  description=get_field_description(field, registry), required=required)
...
def convert_lazy_field_to_dynamic(field, registry=None, executor: ExecutorEnum = ExecutorEnum.SYNC):
    ...
    def dynamic_type():
        ...
        return graphene.Field(
            _type,
            resolver=field_resolver if field_resolver else (
                lazy_resolver if executor == ExecutorEnum.SYNC else lazy_resolver_async),
            description=get_field_description(field, registry), required=required,
        )
